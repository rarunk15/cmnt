...
function _tag_element(frag)
{
	...
	if( close < 0 ) return -1;
	if( frag == lastfrag ) { alert("RECURSION: " + frag.str); return null; }
	lastfrag = frag;
...
function _attributiono(str)
{
	var all = new Array();
	while(1)
	{
		var eq = str.indexOf("=");
		if(str.length == 0 || eq == -1 || eq == (str.length-1))
		{
			return all;
		}
...
function _attribution(str)
{
	...
	while(1)
	{
		...
		if( delim != 0 ) {
		    ...
		    var val1 = str.indexOf( delim, eq+2);
		    if( val1 < 0 ) {
			val = str.substring(eq+2);
		    }
		    else {
			val = str.substring(eq+2, val1);
			nextid = val1+1;
		    }
		}
		else {
		    // look for white space (or end of string)
		    var val1 = str.indexOf( ' ', eq+1);
		    var val2 = str.indexOf( '\t', eq+1);
		    var val;
		    if( val1 != 0 && (val2 == -1 || val1 < val2)) {
			val = str.substring(eq+1,val1);
			nextid = val1+1;
		    }
		    else if( val2 != 0 && (val1 == -1 || val2 < val1)) {
			val = str.substring(eq+1,val2);
			nextid = val2+1;
		    }
		    else
			val = str.substring(eq+1);
		}
		all[name] = _entity(val);
		if( nextid == -1 ) break;
		str = str.substring(nextid,str.length);
	}
	return "";
}
////////////////////
